...
(function () {
    ...
    `;
        ...
        showPlayerList() {
            ...
            const headers = [
                ...
                'Rang',
                'Punten',
                '<img src="https://gpnl.innogamescdn.com/images/game/autogenerated/layout/layout_095495a.png" alt="Steden" width="20" height="20">',
                '<img src="https://gpnl.innogamescdn.com/images/game/overviews/culture_25x25.png" alt="cultuurpunten" width="20" height="20">',
                'Status',
            ...
            players.forEach(player => {
                ...
                html += '<tr>';
                html += `<td><a class="player-name-link" href="#" data-player="${player.name}" data-player-id="${player.id}">${player.name}</a>
                        <div class="player-id">ID: ${player.id}</div>
                        </td>`;
                html += `<td>${player.rank}</td>`;
                html += `<td>${player.points}</td>`;
                html += `<td><div class="icon town">${player.cities}</div></td>`;
                // Cultuurpunten kolom
                html += `<td><div class="icon culture">${player.culturePoints || 0}</div></td>`;
                // Status icoon
                html += `<td>${this.getStatusIcon(statusText)}</td>`;
        ...
        determineStatus(statusImg) {
            ...
            const now = Date.now();
            return {
                "green.png": {type: "active", timestamp: now - (10 * 60 * 1000)}, // <10 min
                "yellow.png": {type: "12h", timestamp: now - (12 * HOUR)},
                "red.png": {type: "24h", timestamp: now - (24 * HOUR)},
                "vacation.png": {type: "vacation"}
            }[statusImg] || {type: "unknown"};
        }
    ...
    .heart {
        background: url('https://example.com/heart-icon.png') no-repeat;
    }
    ...
    .square.green { background: #00ff00; border-color: #00cc00; }
    .square.orange { background: #ff9900; border-color: #cc6600; }
    .square.gray { background: #cccccc; border-color: #999999; }

    .umbrella {
        background: #00f url('https://example.com/umbrella-icon.png') center no-repeat;
        background-size: 80%;
    }
    `;
